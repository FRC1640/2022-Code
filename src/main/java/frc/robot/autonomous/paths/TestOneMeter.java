...
public class TestOneMeter implements PathInterface{

    PathPlannerTrajectory testPath = PathPlanner.loadPath("5 ball new 1", 0.5,  0.25);
    PathPlannerTrajectory testPath2 = PathPlanner.loadPath("Test2", 0.5,  0.25);
    ...
    PathPlannerState testPathState = new PathPlannerState();
    PathPlannerState test2PathState = new PathPlannerState();
    ...
    public Command getAutoCommand(){
        ...
        testPathState = testPath.getInitialState();
        Pose2d testPathPose = new Pose2d(testPathState.poseMeters.getTranslation(), testPathState.holonomicRotation);
        test2PathState = testPath2.getInitialState();
        Pose2d testPathPose2 = new Pose2d(test2PathState.poseMeters.getTranslation(), test2PathState.holonomicRotation);
        ...
        PPSwerveControllerCommand threeBallController = new PPSwerveControllerCommand(testPath,
        ...
        PPSwerveControllerCommand ballController = new PPSwerveControllerCommand(testPath2,
        swerve::getPose, // Functional interface to feed supplier
        kDriveKinematics, new PIDController(0.3, 0.0, 0.0), new PIDController(0.3, 0.0, 0.0), thetaController,
        swerve::setModuleStates, swerve);
        ...
        Command setOffset = new SetOffset(swerve, shooterSubsystem, 0);
        Command pointWheels = new PointWheels(swerve, 0);
        System.out.println("Time: " + testPath.getTotalTimeSeconds());
        Command resetOdo = new ResetOdometry(swerve, testPathPose);
        Command stop = new Stop(swerve);
        swerve.field.getObject("traj").setTrajectory(testPath);
        Command resetGyro = new ResetGyro(swerve, shooterSubsystem);
        SequentialCommandGroup testGroup = new SequentialCommandGroup(resetGyro, resetOdo, setOffset);
